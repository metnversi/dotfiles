...
prepare(){
    mkdir -p ${HOME}/{.local/bin,repos,Downloads,.dev}
...
deployManifest(){
    while IFS='|' read -r filename operation destination; do
        [[ -z "$filename" ]] && continue
        if [[ $filename =~ ^# ]]; then
            echo -e "\033[33m[DELETE]\033[0m $HOME/$destination"
...
emacsrun () {
    ...
    EMACS_PATH="$HOME/.local/bin/emacsscript.sh"
    cat > $EMACS_PATH <<'EOF'
#!/bin/env bash
